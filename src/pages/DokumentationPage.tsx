...
const TEST_ACCOUNTS = [
  {
    role: 'User',
    email: 'test-user@exclusive-ruegen.de',
    password: 'TestUser2024!',
    route: '/portal',
    badge: 'bg-[hsl(var(--brand-gold))]/15 text-[hsl(var(--brand-gold))]',
  },
  {
    role: 'Admin',
    email: 'test-admin@exclusive-ruegen.de',
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      onClick={handleCopy}
      aria-label="Kopieren"
      className="ml-1.5 inline-flex items-center justify-center h-5 w-5 rounded text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"
    >
      ...
      <section className="border-b border-border bg-card">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <div className="flex items-center gap-2 mb-4">
            <KeyRound className="h-4 w-4 text-[hsl(var(--brand-gold))]" />
            <span className="text-xs uppercase tracking-[0.25em] text-muted-foreground font-medium">Test-Zugänge</span>
          </div>
          <div className="grid sm:grid-cols-2 gap-3">
            {TEST_ACCOUNTS.map((acc) => (
              <div key={acc.role} className="border border-border rounded-md p-4 bg-background flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-bold uppercase tracking-[0.2em] px-2 py-0.5 rounded ${acc.badge}`}>
                    {acc.role}
                  </span>
                  <Link
                    to={acc.route}
                    className="text-[10px] uppercase tracking-[0.15em] text-[hsl(var(--brand-gold))] hover:underline"
                  >
                    {acc.route} →
                  </Link>
                </div>
                <div className="space-y-1 font-mono text-xs">
                  <div className="flex items-center">
                    <span className="text-muted-foreground w-20 shrink-0 font-sans">E-Mail</span>
                    <span className="text-foreground">{acc.email}</span>
                    <CopyButton text={acc.email} />
                  </div>
                  <div className="flex items-center">
                    <span className="text-muted-foreground w-20 shrink-0 font-sans">Passwort</span>
                    <span className="text-foreground">{acc.password}</span>
